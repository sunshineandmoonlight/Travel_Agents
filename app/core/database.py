...
async def init_database():
    ...
    # 检查是否禁用数据库（用于测试）
    skip_db = os.getenv("SKIP_DATABASE", "false").lower() == "true"

    if skip_db:
        logger.warning("⚠️ 数据库初始化已跳过 (SKIP_DATABASE=true)")
        return
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"⚠️ Redis连接失败，但应用将继续运行: {e}")
        ...
        logger.warning(f"⚠️ 数据库连接失败，应用将以无数据库模式运行: {e}")
        # 不抛出异常，允许应用继续启动
...
def get_mongo_db_sync() -> Database:
    ...
    注意：每次调用时都会创建新客户端以确保使用最新的配置
    这可能会稍微影响性能，但确保配置更改能立即生效
    """
    # 直接创建新客户端，使用当前settings
    # 这样确保使用最新的环境变量配置
    client = MongoClient(
        settings.MONGO_URI,
        maxPoolSize=settings.MONGO_MAX_CONNECTIONS,

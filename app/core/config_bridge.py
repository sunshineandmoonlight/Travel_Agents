...
def _bridge_system_settings() -> int:
    ...
    try:
        ...
        try:
            logger.info(f"🔄 [config_bridge] 准备同步系统设置到文件系统，包含 {len(system_settings)} 项")
            from app.core.unified_config import unified_config
            result = unified_config.save_system_settings(system_settings)

            if result:
                logger.info(f"✅ [config_bridge] 系统设置同步成功")
            else:
                logger.warning(f"⚠️ [config_bridge] 系统设置同步返回 False")
        except Exception as e:
            logger.warning(f"❌ [config_bridge] 同步系统设置到文件系统失败: {e}")

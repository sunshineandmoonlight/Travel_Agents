...
[!] DEPRECATED: 此模块已废弃，将在 2026-03-31 后移除
...
class ConfigManager:
    ...
    def _load_env_file(self):
        ...
        if env_file.exists():
            # [工具] [修复] override=False 确保环境变量优先级高于 .env 文件
            # 这样 Docker 容器中的环境变量不会被 .env 文件中的占位符覆盖
            logger.info(f"[查询] [ConfigManager] 加载 .env 文件: {env_file}")
            logger.info(f"[查询] [ConfigManager] 加载前 DASHSCOPE_API_KEY: {'有值' if os.getenv('DASHSCOPE_API_KEY') else '空'}")

            load_dotenv(env_file, override=False)

            logger.info(f"[查询] [ConfigManager] 加载后 DASHSCOPE_API_KEY: {'有值' if os.getenv('DASHSCOPE_API_KEY') else '空'}")
    ...
    def _init_mongodb_storage(self):
        """初始化MongoDB存储"""
        logger.info("[工具] [ConfigManager] 开始初始化 MongoDB 存储...")

        if not MONGODB_AVAILABLE:
            logger.warning("[!] [ConfigManager] pymongo 未安装，无法使用 MongoDB 存储")
            return
        ...
        logger.info(f"[查询] [ConfigManager] USE_MONGODB_STORAGE={use_mongodb_env} (解析为: {use_mongodb})")

        if not use_mongodb:
            logger.info("[信息] [ConfigManager] MongoDB 存储未启用，将使用 JSON 文件存储")
            return
        ...
        try:
            ...
            logger.info(f"[查询] [ConfigManager] MONGODB_CONNECTION_STRING={'已设置' if connection_string else '未设置'}")
            logger.info(f"[查询] [ConfigManager] MONGODB_DATABASE_NAME={database_name}")

            if not connection_string:
                logger.error("[FAIL] [ConfigManager] MONGODB_CONNECTION_STRING 未设置，无法初始化 MongoDB 存储")
                return

            logger.info(f"[处理中] [ConfigManager] 正在创建 MongoDBStorage 实例...")
            ...
            if self.mongodb_storage.is_connected():
                ...
                self.mongodb_storage = None
                logger.warning("[!] [ConfigManager] MongoDB连接失败，将使用JSON文件存储")
                ...
                for model in models:
                    ...
                    if model.provider.lower() == "openai":
                        ...
                        if not openai_enabled:
                            model.enabled = False
                            logger.info(f"[锁定] OpenAI模型已禁用: {model.model_name}")
    ...
    def add_usage_record(self, provider: str, model_name: str, input_tokens: int,
        ...
        # [查询] 详细日志：记录保存位置
        logger.info(f"[存储] [Token记录] 准备保存: {provider}/{model_name}, 输入={input_tokens}, 输出={output_tokens}, 成本=¥{cost:.4f}, session={session_id}")

        # 优先使用MongoDB存储
        if self.mongodb_storage and self.mongodb_storage.is_connected():
            logger.info(f"[数据] [Token记录] 使用 MongoDB 存储 (数据库: {self.mongodb_storage.database_name}, 集合: {self.mongodb_storage.collection_name})")
    ...
    def set_openai_enabled(self, enabled: bool):
        ...
        self.save_settings(settings)
        logger.info(f"[工具] OpenAI模型启用状态已设置为: {enabled}")

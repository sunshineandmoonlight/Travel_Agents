...
def init_logging(config_override: Optional[dict] = None) -> None:
    ...
    # 记录初始化信息
    logger.info("[启动] TradingAgents-CN 日志系统初始化完成")
    logger.info(f"[目录] 日志目录: {logger_manager.config.get('handlers', {}).get('file', {}).get('directory', 'N/A')}")
    logger.info(f"[级别] 日志级别: {logger_manager.config.get('level', 'INFO')}")

    # Docker环境特殊处理
    if logger_manager.config.get('docker', {}).get('enabled', False):
        logger.info("[Docker] Docker环境检测到，使用容器优化配置")

    # 记录环境信息
    logger.debug(f"[Python] Python版本: {sys.version}")
    logger.debug(f"[目录] 工作目录: {os.getcwd()}")
    logger.debug(f"[环境] 环境变量: DOCKER_CONTAINER={os.getenv('DOCKER_CONTAINER', 'false')}")
...
def log_startup_info():
    ...
    logger.info("=" * 60)
    logger.info("[启动] TradingAgents-CN 启动")
    logger.info("=" * 60)

    # 系统信息
    import platform
    logger.info(f"[系统] 系统: {platform.system()} {platform.release()}")
    logger.info(f"[Python] Python: {platform.python_version()}")
...
def log_shutdown_info():
    ...
    logger.info("=" * 60)
    logger.info("[关闭] TradingAgents-CN 关闭")
    logger.info("=" * 60)

<template>
  <div class="network-status" v-if="showStatus && !isTravelSystem">
    <el-alert
...
// 检查是否是旅行系统页面（旅行系统使用模拟数据，不需要后端）
const isTravelSystem = computed(() => {
  return route.path.startsWith('/travel')
})

// 只在有网络问题时显示状态（且不是旅行系统）
